...
- [ ] T005 Rewrite `scripts/check-no-notifications.sh` so it permits exactly one notification path and forbids every other: no badge API, no raw constructor, no second notification module, and no notification capability beyond the one declared. Record in the script's own header why the guarantee moved from absence to proof. **Declare `tauri-plugin-notification` in `src-tauri/Cargo.toml` in this same task** — optional and gated behind the `app` feature, exactly as `tauri` is, so the pure layers keep building with `--no-default-features`. The dependency and the rewritten guard are a matched pair: landing the dependency first leaves the branch red, and a branch that sits red stops being read
...
- [ ] T008 Plant a violation against each of the four modified guards, confirm each one fails, then remove the violation — the same standard slice `002` set for its seven checks. Record the confirmation in `specs/003-reflection-and-history/quickstart.md`. **The notification guard is already demonstrated**: T001's spike failed it with `src-tauri/Cargo.toml declares tauri-plugin-notification`, which is exactly the violation it exists to catch. The **ambient-counts guard is also demonstrated** — T002 verified it against ten planted cases, seven failing and three passing, recorded in `quickstart.md`. **Both ESLint rules are also demonstrated** — T003 and T004 together verified them against a 17-case matrix covering reach data, journal entries, the three browser notification routes, and the plugin import. That leaves only the *rewritten* notification guard (T005), which is a different check from the one that fired in T001 and needs its own planted violation against its new rule. **Keep the matrix and re-run it whenever an allowlist or a restricted module changes** — it has now caught a hole in each of the three guards it was pointed at, including one introduced by the task immediately before
